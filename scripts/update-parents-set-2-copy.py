# -*- coding: utf-8 -*-
"""
Update Parents Set 2 ("Behind the Champion") copy across all 42 Buffer posts.
Uses Buffer GraphQL editPost mutation to preserve existing scheduled timestamps
and assets with zero duplicates.
"""
...
Mothers, women mentors, and family leaders: When was the last time you had the courage to step back so your child could develop their own wings? Knowing when to release control is the ultimate test of parental wisdom.
...
Men and fathers: Delayed gratification is the ultimate mark of mature leadership. Playing the long game with your children always outperforms chasing short-term applause. Are you willing to resist the rush?
...
Mothers, women leaders, and mentors: What is one boundary you've had to hold firmly to protect your young athlete's character from the noise of early fame?
...
Women and mothers: Are you modeling the kind of quiet, steady peace in private that you want your children to walk with in public?
...
To every parent stretching their budget for a child's dream: What was the hardest sacrifice you made in silence that nobody else ever saw?
...
Parents and mentors: What is the makeshift "backyard" project you are building right now to invest in your child's future?
...
Fathers and male mentors: When your daughter faces high-pressure moments, does she experience you as her fiercest critic, or her safest harbor?
...
Fathers and mentors: What emotional energy are you projecting onto your young athletes when the championship pressure spikes?
...
Mothers on the sidelines: What is the most meaningful moment of quiet encouragement you've ever shared with your child after a heartbreaking loss?
...
Fathers and mentors: What is the single most enduring lesson your own father passed down to you that guides how you lead your family today?

Never underestimate the power of your faithful presence in your child's life.
...
Coaches, athletic directors & sports leaders: How does your program actively partner with and honor the parents who sacrifice everything to put champions on your roster?
...
def execute_edit_post(buffer_post_id, new_text):
    variables = {
        "input": {
            "id": buffer_post_id,
            "text": new_text
    ...
    data = json.dumps({"query": EDIT_POST_MUTATION, "variables": variables}).encode('utf-8')
...
def main():
    print("=================================================================")
    print("🚀 Updating Parents Set 2 ('Behind the Champion') Copy in Buffer")
    print("=================================================================\n")

    report_path = r"c:\Users\Zhane\Documents\New project\LD 2.0 WEBSITE\scripts\parents-set-2-scheduled-report.json"
    ...
    results_by_id = {r["id"]: r for r in report.get("results", [])}
    posts_copy_by_id = {p["id"]: p for p in posts_copy}

    updated_count = 0
    total = len(posts_copy)

    for idx, post in enumerate(posts_copy, 1):
        pid = post["id"]
        report_item = results_by_id.get(pid)
        if not report_item or not report_item.get("bufferPostId"):
            print(f"[{idx}/{total}] ⚠️ Post #{pid} missing bufferPostId. Skipping.")
            continue

        buffer_id = report_item["bufferPostId"]
        new_text = post["text"]

        while True:
            print(f"[{idx}/{total}] Updating Post #{pid} (Buffer: {buffer_id})...", end="", flush=True)
            res = execute_edit_post(buffer_id, new_text)

            if res.get("status_code") == 429 or "429" in str(res.get("error", "")):
                wait_sec = res.get("retry_after", 60)
                print(f" [RATE LIMIT] Sleeping {wait_sec + 3}s...")
            ...
            if typename == "PostActionSuccess":
                print(f" ✅ SUCCESS (Status: {edit_data.get('post', {}).get('status')})")
                report_item["updated_copy"] = True
                report_item["headline"] = new_text.splitlines()[0]
                updated_count += 1
                break
            else:
        ...
        # Checkpoint update to report file
        with open(report_path, "w", encoding="utf-8") as f:
            json.dump(report, f, indent=2)

        time.sleep(1.2)  # Respect Buffer burst rate limits

    print("\n=================================================================")
    print(f"🏁 Completed: {updated_count}/{total} posts successfully updated in Buffer!")
    print("=================================================================\n")
