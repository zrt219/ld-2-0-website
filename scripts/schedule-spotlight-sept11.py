...
manifest_path = os.path.join(os.path.dirname(__file__), 'spotlight-sept11-manifest.json')
report_path = os.path.join(os.path.dirname(__file__), 'spotlight-sept11-scheduled-report.json')

with open(manifest_path, 'r', encoding='utf-8') as f:
    spotlight_posts = json.load(f)
...
def schedule_spotlight_campaign():
    print("======================================================")
    print("🌟 Scheduling September 11 Spotlight Campaign (3 Posts)")
    print(f"Channel: Lornette Daye LinkedIn ({CHANNEL_ID})")
    print("Window: Friday, September 11, 2026 (8:30 AM, 1:30 PM, 6:30 PM MDT)")
    print("======================================================\n")

    results = []

    for idx, p in enumerate(spotlight_posts):
        if p.get("status") == "scheduled" and p.get("bufferPostId"):
            print(f"[{idx + 1}/{len(spotlight_posts)}] Post #{p['id']} already scheduled (ID: {p['bufferPostId']})")
            results.append(p)
            continue

        print(f"[{idx + 1}/{len(spotlight_posts)}] Scheduling Post #{p['id']} ({p['slot']}): \"{p['title']}\"")
        ...
        input_data = {
            ...
            "assets": [{"image": {"url": p["assetUrl"]}}] if p.get("assetUrl") else []
        }
        ...
        with open(manifest_path, "w", encoding="utf-8") as f:
            json.dump(spotlight_posts, f, indent=2, ensure_ascii=False)
    ...
    print(f"Summary: {scheduled_count}/{len(spotlight_posts)} posts successfully scheduled directly in Buffer.\n")

if __name__ == "__main__":
    schedule_spotlight_campaign()
