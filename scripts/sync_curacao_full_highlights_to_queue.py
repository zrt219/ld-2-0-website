...
with open('scripts/curacao-full-highlights-scheduled-report.json', 'r', encoding='utf-8') as f:
    highlights_posts = json.load(f)

existing_ids = {p.get('postId') or p.get('bufferPostId') for p in master_queue}

added = 0
for cp in highlights_posts:
    pid = cp.get('postId')
    if pid and pid not in existing_ids:
        master_queue.append({
            'campaign': 'curacao-full-highlights',
            'id': f"curacao-hl-{cp['id']:02d}",
...
print(f"Added {added} Curaçao full highlights posts to master queue. Total items: {len(master_queue)}")
