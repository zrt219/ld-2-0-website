...
with open('scripts/curacao-ads-scheduled-report.json', 'r', encoding='utf-8') as f:
    ad_posts = json.load(f)
...
for cp in ad_posts:
    ...
    if pid and pid not in existing_ids:
        master_queue.append({
            'campaign': 'curacao-ads',
            'id': f"curacao-ad-{cp['id']:02d}",
...
print(f"Added {added} Curaçao ad posts to master queue. Total items: {len(master_queue)}")
